...
- **Frontend `essensys-support-site`** : ajout d'un champ « Confirmer le mot de passe » dans `Register.jsx` pour supprimer la cause racine (faute de frappe silencieuse à l'inscription).
...
- `auth-recovery-ui`: parcours UI « mot de passe oublié / réinitialisation » sur `mon.essensys.fr`, incluant la confirmation de mot de passe à l'inscription
