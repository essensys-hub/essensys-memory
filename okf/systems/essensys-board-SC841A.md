---
type: Repository
title: Essensys Board Sc841a
description: "Carte **banc de test usine** « Banc BP » : teste automatiquement les cartes de type BP (alimentations, alarmes, téléinfo, façade) en production."
resource: file:///Users/nrineau/ESSENSYS/essensys-board-SC841A
tags: [essensys, repository, firmware, legacy]
timestamp: 2026-06-28T19:07:32Z
repo: essensys-board-SC841A
layer: firmware
era: legacy
source_wiki: ../../wiki/entities/essensys-board-sc841a.md
---
<!-- BEGIN GENERATED CONTENT -->
# Rôle

Carte **banc de test usine** « Banc BP » : teste automatiquement les cartes de type BP (alimentations, alarmes, téléinfo, façade) en production.

# Interfaces

* Couche : Firmware / armoire.
* Era : legacy.
* Dépôt local : `essensys-board-SC841A`.
* Détaillé dans [architecture armoire](/synthesis/armoire-architecture.md).
* Lié à [Table D Echange](/protocols/table-d-echange.md).
* Contraintes legacy : compatibilité firmware et non-régression protocolaire.

# Dépendances

* Détaillé dans [architecture armoire](/synthesis/armoire-architecture.md).
* Lié à [Table D Echange](/protocols/table-d-echange.md).
* Contraintes legacy : compatibilité firmware et non-régression protocolaire.

# Code pointers

* `essensys-board-SC841A/README.md`

# Risques

* Ne pas inférer de changement fonctionnel depuis cette fiche : elle décrit l'état et les contrats connus.
* Toute zone legacy ou protocolaire doit être vérifiée contre firmware + backend avant modification.

# Citations

[1] [Inventory](../../output/okf-repository-inventory.json)
[2] [Wiki source](../../wiki/entities/essensys-board-sc841a.md)
<!-- END GENERATED CONTENT -->
