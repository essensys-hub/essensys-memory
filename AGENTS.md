...
> Documentation technique et architecture du projet ESSENSYS — domotique, firmware, backends, déploiement, protocoles IoT
...
## ESSENSYS-Specific Rules

11. Run `scripts/sync-sources.sh`, `scripts/extract-git-history.sh`, and `scripts/update-roadmap.sh` after any OpenSpec change or significant merge.
12. Never modify files in `raw/` during ingest — only sync scripts may refresh `raw/architecture/` and `raw/openspec-index/`.
13. Tag legacy entities with `era: legacy` (MQX, ASP.NET, BP_MQX_ETH). Tag modern stack with `era: modern`.
14. OpenSpec changes live in their host repo; this vault aggregates them via `wiki/roadmap/` and `raw/openspec-index/manifest.json`.
