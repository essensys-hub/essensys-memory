...
- **MCU** : Microchip **dsPIC33FJ256GP710A** (DSC 16 bits, boîtier 100 broches), oscillateur primaire HS, watchdog désactivé par soft.
- **Périphériques utilisés** (modules firmware) : ADC pour les mesures alim/variateurs, **I2C** et **SPI** (pilotage d'expanders / cartes filles du banc), **UART** (report des résultats sur hyperterminal), EEPROM I2C externe (`i2cEmem.h`).
- **Toolchain** : MPLAB X / XC16 (`MP_PROCESSOR_OPTION=33FJ256GP710A`, linker `p33FJ256GP710A.gld`).
...
- **Compilation** : ouvrir `Banc_BA_V3.X` (ou `Banc_BA_V3_Fast.X`) dans **MPLAB X IDE** avec **XC16** ; `Makefile` fourni. La build par défaut est en mode debug (`-D__DEBUG`, PICkit 3).
- **Flash** : programmer le `.hex` de production sur le dsPIC via PICkit 3.
- **Utilisation** : lancement du test manuel (barre espace) ou automatique à la fermeture du banc ; résultats et numéro de version logicielle affichés sur hyperterminal (UART).

## Intégrations (comment la carte communique)
- **UART** vers un PC / hyperterminal : affichage des résultats de test et de la version du soft.
- **I2C / SPI** : pilotage des expanders et de la carte sous test (contacts secs, lecture d'états relais), + EEPROM I2C.
- **ADC** : mesures analogiques (alim, niveaux de variateurs) pour les critères de validation.
- Interface mécanique/électrique avec la carte BA insérée dans le banc.
